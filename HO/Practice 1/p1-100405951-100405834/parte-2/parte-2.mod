...
minimize gastos_extras: sum{a in AVION, f in FRANJA, p in PISTA} avion_franja_pista[a,f,p]*coste[a]*(inicio[f]-hora_programada[a]);
...
s.t. noAntesDeInicio{a in AVION, f in FRANJA, p in PISTA} : (1440*avion_franja_pista_i[a,f,p])+inicio[f]*avion_franja_pista[a,f,p] >= hora_programada[a];
  /*El inicio del slot de aterrizaje debe ser igual o anterior a la hora l´ımite de aterrizaje del avion.*/
s.t. AntesDeHoraLimite{a in AVION,f in FRANJA, p in PISTA}: (1440*avion_franja_pista_i[a,f,p])+hora_limite[a]*avion_franja_pista[a,f,p] >= inicio[f];
  /*Por cuestiones de seguridad, no se pueden asignar dos slots consecutivos en la misma pista.*/
s.t. noConsecutivos{a in AVION, b in AVION, f in FRANJA, g in FRANJA, p in PISTA : a<>b and inicio[f]>inicio[g]} : (1455*avion_franja_pista_i[a,f,p])+inicio[f]*avion_franja_pista[a,f,p]-inicio[g]*avion_franja_pista[b,g,p] >= 16;
  /*Inversa de avion_franja_pista*/
s.t. invesaAvionFranjaPista{a in AVION,f in FRANJA, p in PISTA} : avion_franja_pista[a,f,p]+avion_franja_pista_i[a,f,p]=1;
