...
public class Ejercicio5 {
	public static void main(String[] args) {
		...
		for (int i = 0; i < disco.getGrupo().getMiembros().length; i++) {
			System.out.println(
				"\t" + disco.getGrupo().getMiembros()[i].getNombre() + " (" + disco.getGrupo().getMiembros()[i].getNacimiento() + ")"
			);
		}
		...
		for (int i = 0; i < disco.getCanciones().length; i++) {
			System.out.println("\t" + (
				i + 1
			) + ": " + disco.getCanciones()[i].getTitulo() + " (" + disco.getCanciones()[i].getDuracion() + " segundos)");
		}
		sc.close();
	}
}
