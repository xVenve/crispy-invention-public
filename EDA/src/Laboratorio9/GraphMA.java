...
public class GraphMA implements IGraph {
	...
	@Override public int getDegree(int i) {
		...
		if (directed) 
			degree = getInDegree(i) + getOutDegree(i);
		else {
	...
	@Override public int getInDegree(int i) {
	...
	@Override public void addEdge(int i, int j, float w) {
	...
	@Override public void removeEdge(int i, int j) {
	...
	@Override public Float getWeightEdge(int i, int j) {
		//checks if the indexes are right
		if (!checkVertex(i)) 
			throw new IllegalArgumentException("Nonexistent vertex  " + i);
		if (!checkVertex(j)) 
	...
	public int[] getAdjacents(int i) {
		...
		int numAdjacents = 0;
		if (directed) 
			numAdjacents = getOutDegree(i);
		else 
			numAdjacents = getDegree(i);
