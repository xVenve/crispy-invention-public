...
//these packages are used to implement the showLevel method the interface for a queue
...
public class BSTNode implements IBSTNode {
  ...
  //returns the depth of the invoking node an iterative version
  public int getDepth() {
    int depth = 0;
    //we need to define a variable node, that allows us to ascend through its ancestors until we reach the root
  ...
  public int getRecDepth1() {
    if (this.parent == null) return 0; else return this.parent.getRecDepth2();
  }

  //Another recursive version 	using an auxiliary method I think that this is more elegant than the previous one
  public int getRecDepth2() {
    return getRecDepth2(this);
  }

  protected static int getRecDepth2(BSTNode node) {
    if (node == null) return 0; else return 1 + getRecDepth2(node.parent);
  }
  ...
  //a recursive version that does not use an auxiliary method I don't like it, but it's right
  public int getSize2() {
    int result;
    if (this.left == null && this.right == null) result = 1; else if (
      this.left != null && this.right != null
    ) result = 1 + this.left.getSize2() + this.right.getSize2(); else if (
      this.left != null
    ) result = 1 + this.left.getSize2(); else result = //(this.right!=null)
      1 + this.right.getSize2();
    return result;
  }

  //return the height of the node (which is the longest path from the node to any leaf)
  ...
  public void showLevels() {
    // we define a queue of BSTNodes. Queue is an interface LinkedList is the implementation Java allows us to create
    // collections of generic types with <>... in our case, we choose BSTNode.
  ...
  public static void main(String args[]) {
    // We want to build the follow tree: 				5 		3				8 					6		9 YOU SHOULD NEVER IMPLEMENT THIS CODE!!! YOU SHOULD ALWAYS
    // USE THE INSERT METHOD OF A TREE HOWEVER, WE DON'T HAVE IT YET.
    ...
    System.out.println(
      "getDepth for " + right1.key + " is=" + right1.getDepth()
    );
    System.out.println("getDepth for " + left2.key + " is=" + left2.getDepth());
    System.out.println(
      "getDepth for " + right2.key + " is=" + right2.getDepth()
    );
    ...
    System.out.println(
      "getHeight for " + left1.key + " is=" + left1.getHeight()
    );
    System.out.println(
      "getHeight for " + right1.key + " is=" + right1.getHeight()
    );
    System.out.println(
      "getHeight for " + left2.key + " is=" + left2.getHeight()
    );
    System.out.println(
      "getHeight for " + right2.key + " is=" + right2.getHeight()
    );
    System.out.println();
