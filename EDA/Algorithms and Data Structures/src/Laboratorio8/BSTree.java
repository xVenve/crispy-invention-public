...
public class BSTree implements IBSTree {
	...
	/**
	 * The method searches the key into the subtree that hangs down from currentNode
	...
	// creates a new node and inserts it into the tree first, it must search its
	// right position into the tree the new nodes
	...
	/**
	 * This method first looks for (recursively) the right position and then inserts
	 * the new node. Note: It would be more correct for the method to be in the
	 * BSTNode class
	 * 
	 * @param newNode
	...
	// searches the node whose key is key and removes it from the tree the method
	// uses a recursive method that returns the
	// modified tree
	...
	private BSTNode remove(Integer key, BSTNode currentNode) {
		...
		// Here, it means that key.compareTo(node.Key)==0 That is, we already have found
		// it: node is the node to remove. First
		...
		// Third case: node has two childs. We can replace its value by the maximum
		// value in its left child or by the minimum
		...
		// Finally, we must remove the sucesorNode (which is one of the two previous
		// cases)
	...
	// Recursive method that visits all nodes and prints those whose parents are
	// multiple of 10.
	public void print10() {
		print10(root);
	}
	...
	protected static int sumAllKeys(BSTNode node) {
		if (node == null)
			...
			return node.key + sumAllKeys(node.left) + sumAllKeys(node.right);
	}
	...
	public static int getFactorSize(BSTNode node) {
		if (node == null)
			...
			return Math.abs(BSTNode.getSize(node.right) - BSTNode.getSize(node.left));
	}
	...
	public static int getFactorHeight(BSTNode node) {
		if (node == null)
			...
			return Math.abs(BSTNode.getHeight(node.right) - BSTNode.getHeight(node.left));
	}
	...
	public static boolean isHeightBalalanced(BSTNode node) {
		if (node == null)
			return true;
		return getFactorSize(node) <= 1 && isHeightBalalanced(node.left) && isHeightBalalanced(node.right);
	}
	...
	public static void main(String args[]) {
		...
		bstree.insert(21, "21");
		bstree.insert(34, "34");
		// 20 12 23 16 21 34 System.out.println(bstree.find(30));
