...
public class GraphMA implements IGraph {
	...
	public int getDegree(int i) {
		...
		if (directed)
			...
			// we could count the number of rows or columns that have an edge with i in this
			// implementation, we count the number of
			// rows
	...
	public static void main(String args[]) {
		...
		// graph.show(); code for creating a graph with random weights Random rn=new
		// Random(); initializes a directed graph
		// with random numbers for (int i=0; i<graph.numVertices;i++) { for (int j=0;
		// j<graph.numVertices;j++) { add an
		// edge with a random weight graph.addEdge(i, j,rn.nextInt(5)); graph.addEdge(i,
		// j,1); } } 0 1 2 3 4
		// 5 6 7
		// 0 - 1 1 - - - - -
		// 1 - - 1 1 - - - -
