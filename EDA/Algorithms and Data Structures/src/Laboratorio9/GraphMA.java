...
public class GraphMA implements IGraph {
  ...
  public GraphMA(int n, int max, boolean d) {
    ...
    if (max <= 0) throw new IllegalArgumentException(
      "Negative maximum number of vertices!!!"
    );
    if (n <= 0) throw new IllegalArgumentException(
      "Negative number of vertices!!!."
    );
    if (n > max) throw new IllegalArgumentException(
      "number of vertices can never be greater than the maximum."
    );
  ...
  private boolean checkVertex(int i) {
    ...
    if (i >= 0 && i < numVertices) return true; else return false;
  }
  ...
  public int getDegree(int i) {
    if (!checkVertex(i)) throw new IllegalArgumentException(
      "Nonexistent vertex  " + i
    );
    int degree = 0;

    if (directed) degree = getInDegree(i) + getOutDegree(i); else {
      // we could count the number of rows or columns that have an edge with i in this implementation, we count the number of
      // rows
  ...
  public void addEdge(int i, int j, float w) {
    ...
    if (!checkVertex(j)) throw new IllegalArgumentException(
      "Nonexistent vertex  " + j
    );
  ...
  public boolean isEdge(int i, int j) {
    ...
    if (matrix[i][j] == null) return false; else return true;
  }
  ...
  public int[] getAdjacents(int i) {
    ...
    //obtains the number of adjacent vertices, which will be the size of the array
    int numAdjacents = 0;
    if (directed) numAdjacents = getOutDegree(i); else numAdjacents =
      getDegree(i);
  ...
  public static void main(String args[]) {
    ...
    //		System.out.println("Vertices:" + graph.sizeVertices()); 		System.out.println("total edges:" + graph.sizeEdges());
    //
    // for (int i=0;i<graph.numVertices;i++) { 			System.out.print("Indeg("+i+")="+graph.getInDegree(i));
    // System.out.print("\tOutdeg("+i+")="+graph.getOutDegree(i)); 			System.out.print("\tDeg("+i+")="+graph.getDegree(i));
    // System.out.println();
    //
    //		}
    graph.breadth();
