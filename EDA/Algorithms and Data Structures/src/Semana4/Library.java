  ...
  public void insertByTitle(String ti, String au, String ge) {
    ...
    if (isEmpty()) {
      ...
      if (!contains(b)) {
        ...
        int index = -1;
        for (
          DNode NodeIt = header.next;
          NodeIt.elem != null && !orden;
          NodeIt = NodeIt.next, index++
        ) {
          salir = false;
          for (
            int i = 0;
            NodeIt.elem.title.length() > i &&
            i < ti.length() &&
            !orden &&
            !salir;
            i++
            ...
            if (
              NodeIt.elem.title.toLowerCase().charAt(i) !=
              ti.toLowerCase().charAt(i) &&
              !repeat
            ) {
              if (
                NodeIt.elem.title.toLowerCase().charAt(i) >
              ...
              repeat = true;
              if (
                NodeIt.elem.title.toLowerCase().charAt(i + 1) >
                ti.toLowerCase().charAt(i + 1)
              ) {
                orden = true;
              } else if (
                NodeIt.elem.title.toLowerCase().charAt(i + 1) ==
    ...
    if (size >= 2) {
      for (int i = 1; i < size; i++) {
        ...
        for (int j = 0; j < size - i; j++) {
          ...
          NodeIt = NodeIt.next;
          if (
            NodeIt.prev.elem.author.toLowerCase().charAt(0) >
            NodeIt.elem.author.toLowerCase().charAt(0)
          ) {
            change = true;
          } else if (
            NodeIt.prev.elem.author.toLowerCase().charAt(0) ==
            NodeIt.elem.author.toLowerCase().charAt(0) &&
            NodeIt.prev.elem.author.toLowerCase().charAt(1) >
            NodeIt.elem.author.toLowerCase().charAt(1)
          ) {
            change = true;
          }
          if (change) {
